...
public class ForceRemoveCmd extends DJCommand
{
    public ForceRemoveCmd(Bot bot)
    {
        ...
        this.help = "удаляет все обьекты пользователя из очереди";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyError("Упомяни пользователя!");
            return;
        }
        ...
        {
            event.replyError("Сейчас пусто в очереди!");
            return;
        }
    ...
    private void removeAllEntries(User target, CommandEvent event)
    {
        ...
        if (count == 0)
        {
            event.replyWarning("**"+target.getName()+"** не имеет песен в очереди!");
        }
        else
        {
            event.replySuccess("Успешно удалено `"+count+"` обьектов из **"+target.getName()+"**#"+target.getDiscriminator()+".");
        }
    }
}
