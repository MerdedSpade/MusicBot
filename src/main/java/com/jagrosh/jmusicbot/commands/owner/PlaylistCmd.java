...
public class PlaylistCmd extends OwnerCommand 
{
    ...
    {
        StringBuilder builder = new StringBuilder(event.getClient().getWarning()+" Команды управления плэйлистами:\n");
    ...
    public class MakelistCmd extends OwnerCommand 
    {
        public MakelistCmd()
        {
            ...
            this.help = "создаёт новый плэйлист";
            this.arguments = "<имя>";
            this.guildOnly = false;
        }
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            if(bot.getPlaylistLoader().getPlaylist(pname)==null)
            {
                ...
                {
                    event.reply(event.getClient().getError()+" Не удалось создать плэйлист: "+e.getLocalizedMessage());
                }
            }
            else
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            else
            {
                try
                {
                    ...
                    event.reply(event.getClient().getSuccess()+" Успешно удалён плэйлист `"+pname+"`!");
                }
                catch(IOException e)
                {
                    event.reply(event.getClient().getError()+" Не удалось удалить плэйлист: "+e.getLocalizedMessage());
                }
            }
        }
    }
    ...
    public class AppendlistCmd extends OwnerCommand 
    {
        public AppendlistCmd()
        {
            ...
            this.help = "добавляет песни в существующий плэйлист";
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            if(parts.length<2)
            {
                event.reply(event.getClient().getError()+" Пожалуйста напишите имя плэйлиста и URLы для добавления!");
                return;
            }
            ...
            else
            {
                ...
                try
                {
                    ...
                    event.reply(event.getClient().getSuccess()+" Успешно добавлено "+urls.length+" вещей в плэйлист `"+pname+"`!");
                }
                catch(IOException e)
                {
                    event.reply(event.getClient().getError()+"Не удалось добавить в плэйлист: "+e.getLocalizedMessage());
                }
            }
        }
    }
    ...
    public class DefaultlistCmd extends AutoplaylistCmd 
    {
        public DefaultlistCmd(Bot bot)
        {
            ...
            this.arguments = "<название плэйлиста|NONE>";
            this.guildOnly = true;
        }
    }
    ...
    public class ListCmd extends OwnerCommand 
    {
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            if(!bot.getPlaylistLoader().folderExists())
            {
                event.reply(event.getClient().getWarning()+" Папка плэйлистов не существует или не была создана!");
                return;
            }
            ...
            if(list==null)
                ...
                event.reply(event.getClient().getWarning()+" Нету плэйлистов в папке плэйлистов!");
            else
            {
